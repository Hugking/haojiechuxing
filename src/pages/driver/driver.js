...
Page({
  data: {
    ...
    addarray: ['出发地', '目的地'],
    // 筛选
    timearray: ['不限', '0:00-7:00', '7:00-18:00', '18:00-23:59'],
  ...
  time_zone_st(zone, today) {
    let zones = zone.split("-")
    if (today) {
      console.log('今天', zone)
      var dates = util.Formatunixdate(Date.parse(new Date()) / 1000) + ' ' + zones[0]
      var datelines = new Date(dates.replace(/-/g, "/"))
      var time = Date.parse(datelines) //日期转时间戳
      return time
    } else {
      console.log('明天', zone)
      var dates = util.Formatunixdate(Date.parse(new Date()) / 1000) + ' ' + '00:00'
      var datelines = new Date(dates.replace(/-/g, "/"))
      var datee = util.Formatunixdate(Date.parse(datelines) / 1000 + 24 * 3600) + " " + zones[0]
      var datelinee = new Date(datee.replace(/-/g, "/"))
      var time = Date.parse(datelinee) //日期转时间戳
      return time
    }
  ...
  time_zone_en(zone, today) {
    let zones = zone.split("-")
    if (today) {
      console.log('今天', zone)
      var dates = util.Formatunixdate(Date.parse(new Date()) / 1000) + ' ' + zones[1]
      var datelines = new Date(dates.replace(/-/g, "/"))
      var time = Date.parse(datelines) //日期转时间戳
      return time
    } else {
      ...
      var dates = util.Formatunixdate(Date.parse(new Date()) / 1000) + ' ' + '00:00'
      var datelines = new Date(dates.replace(/-/g, "/"))
      var datee = util.Formatunixdate(Date.parse(datelines) / 1000 + 24 * 3600) + " " + zones[1]
      var datelinee = new Date(datee.replace(/-/g, "/"))
      var time = Date.parse(datelinee) //日期转时间戳
      return time
    }
    // if (today) { console.log('今天', zone) } else { console.log('明天', zone) }
    // let zones = zone.split("-")
  ...
  Order_info(type) {
    ...
    if (time) {
      query.compare('pre_time_str', '>=', this.time_zone_st(time, true));
      query.compare('pre_time_str', '<', this.time_zone_en(time, true));
  ...
  get_order(e) {
    ...
    Driver.setQuery(query.contains('openid', this.data.userInfo.openid)).find().then(res => {
      ...
      if (res.data.objects[0]) {
        let _driver = res.data.objects[0]
        if (res.data.objects[0].can_get) {
          Order.get(e.currentTarget.id).then(res => {
            ...
            if (res.data['confirm_isget'] || res.data['isget']) {
              ...
              //   })
              // }
              if (this.time_zone_st('7:00-18:00', true) < res.data['pre_time_str'] && res.data['pre_time_str'] < this.time_zone_en('7:00-18:00', true) || this.time_zone_st('7:00-18:00', false) < res.data['pre_time_str'] && res.data['pre_time_str'] < this.time_zone_en('7:00-18:00', false)) {
                console.log('白班')
                if (_driver.type == 'night') {
                  wx.showToast({
                    title: '由于您注册的是夜班司机，暂无法查看白班订单',
                    icon: 'none',
                    duration: 2000
                  })
                  wx.hideNavigationBarLoading()
                ...
                if (this.time_zone_st('0:00-7:00', true) < res.data['pre_time_str'] && res.data['pre_time_str'] < this.time_zone_en('0:00-7:00', true) || this.time_zone_st('0:00-7:00', false) < res.data['pre_time_str'] && res.data['pre_time_str'] < this.time_zone_en('0:00-7:00', false) || this.time_zone_st('18:00-23:59', true) < res.data['pre_time_str'] && res.data['pre_time_str'] < this.time_zone_en('18:00-23:59', true) || this.time_zone_st('18:00-23:59', false) < res.data['pre_time_str'] && res.data['pre_time_str'] < this.time_zone_en('18:00-23:59', false)) {
                  console.log('夜班')
                  if (_driver.type == 'light') {
                    wx.showToast({
                      title: '由于您注册的是白班司机，暂无法查看夜班订单',
                      icon: 'none',
                      duration: 2000
                    })
                    wx.hideNavigationBarLoading()
