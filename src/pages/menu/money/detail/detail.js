      ...
      if (res.data.objects[0]) {
        ...
        if (!res.data.objects[0].code) {
          ...
          if (this.data.cost == 0.00) {
            ...
            // amount.set("total_cost", util.accAdd(parseFloat(res.data.objects[0].total_cost).toFixed(2), parseFloat(this.data.cost).toFixed(2) ))
            amount.set("no_cost", util.accAdd(res.data.objects[0].no_cost, this.data.cost))    
            // amount.set("now_cost", res.data.objects[0].now_cost - this.data.cost)
            ...
            amount.update().then(res => {
              ...
              console.log('更新的余额', res.data.total_cost, res.data.now_cost, res.data.now_cost)
              wx.showToast({
                title: '提现申请已提交',
                icon:'none'
              })
