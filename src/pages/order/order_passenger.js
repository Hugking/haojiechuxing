    ...
    if (that.data.orderlist[e.currentTarget.dataset.idx].confirm_isget) {
      ...
      if (!e.currentTarget.dataset.target) {
        ...
        order.update().then(res => {
          ...
          if (!that.data.orderlist[e.currentTarget.dataset.idx].pay_status) {
            wx.showToast({
              title: '已取消',
              icon: 'success',
              duration: 2000
  ...
  reminder(e) {
    ...
    if (that.data.orderlist[e.currentTarget.dataset.idx].confirm_isget) {
      ...
      Order.get(e.currentTarget.id).then(res => {
        // success
        if((this.data.nowtime - res.data.created_at)<600){
          wx.showToast({
            title: '下单10分钟后才可催单',
          })
        }
    ...
    if (that.data.orderID){
      ...
      order.update().then(res => {
        ...
        wx.BaaS.invoke('send_admin', {
          ...
          "data": util.Formatunix(that.data.orderlist[that.data.orderIDX].pre_time_str / 1000) + ' ' + that.data.orderlist[that.data.orderIDX].startname + "=>" + that.data.orderlist[that.data.orderIDX].endname,
          "time": util.Formatunix(Date.parse(new Date()) / 1000),
          "cost": res.data.cost,
          "name": res.data.username,
          "reason": that.data.cancelcontext
        }).then(res => {
        ...
        wx.BaaS.invoke('pay_black', { "trade_no": res.data['trade_no'] }).then(res => {
          console.log(res)
        })
